...
try:
    ...
    print("[ERROR] ❌ Invalid timezone. Falling back to Asia/Kolkata.")
...
def get_current_show_and_start_time():
    ...
    for day_offset in [0, -1]:
        ...
        for i, entry in enumerate(SCHEDULE):
            ...
            naive_end = datetime.combine(check_date, datetime.strptime(SCHEDULE[next_i]["start"], "%H:%M").time())
    ...
    print("[WARN] ⚠️ No current show found in schedule. Defaulting to first show.")
    fallback_show = SCHEDULE[0]
    return fallback_show["show"], PLAYLISTS.get(fallback_show["show"], {}), None, 3600
...
def start_stream_for_show(show_name, show_info, show_start_time, show_duration_seconds):
    ...
    if logo_exists:
        ...
        print("[INFO] 🌾 Logo found. Applying overlay.")
    ...
    current_process = subprocess.Popen(ffmpeg_cmd, stderr=subprocess.PIPE)
...
def manage_stream():
    ...
    try:
        ...
        print("[INFO] 🔐 Another stream manager is already running.")
        return
...
@app.route("/")
def home():
    return f"<h1>📺tv {CHANNEL_NAME} is live!</h1><p>Access the stream at <a href='/stream/live.m3u8'>/stream/live.m3u8</a></p>"
...
def start_stream_manager():
    thread = threading.Thread(target=manage_stream, daemon=True)
    thread.start()

start_stream_manager()  # Start manager regardless of __main__ or gunicorn
